...
The `name` and `requestCodes` attributes are the same. The `permissions` will need to be replaced with list of `@Permission` annotations, each containing a list of manifest strings and their corresponding `alias`, which you can omit for now until the new permissions API is implemented in your plugin. The `permissionRequestCode` attribute is removed.
...
> Request codes are no longer used for permission requests in Android. Capacitor 3.0 uses the new AndroidX Activity Result APIs to handle permission requests using a callback. Capacitor may be extending this implementation to include Activiy results in the near future.
