...
const uniqueId = Math.random().toString().replace(".", "");

const DocsSearch: React.FC<Props> = ({ theme, placeholder }) => {
  ...
  const getContentStats = useCallback(() => {
    requestAnimationFrame(() => {
      ...
      let left =
        siteContent.getBoundingClientRect().left -
    ...
    window.docsearch({
      ...
      queryHook: () => {
        if (input.isPristine) {
          ...
          input.el = el.current.querySelector(
            `#id-${uniqueId} input[name="search"]`
          ) as HTMLInputElement;
