...
class App {
    ...
    */
    ...
    __stencil_render() {
        return (h("site-root", null, h("div", { id: "main-div" }, h("site-platform-bar", { productName: "Capacitor" }), h("capacitor-site-header", null), h("div", { class: "app root" }, h(Router.Switch, null, h(Route, { path: "/" }, h("landing-page", null)), h(Route, { path: "/blog" }, h("blog-page", null)), h(Route, { path: match('/blog/:slug'), render: ({ slug }) => (h("blog-page", { slug: slug })) }), h(Route, { path: "/enterprise" }, h("capacitor-enterprise", null)), h(Route, { path: "/docs" }, h("document-component", { page: "/docs/" })), h(Route, { path: match('/docs/:pageName*'), render: ({ pageName }) => (h("document-component", { page: `/docs/${pageName}` })) }))))));
    }
    ...
    static get cmpMeta() { return {
        ...
        "$members$": {
            "isLandingPage": [4, "is-landing-page"],
            "isLeftSidebarIn": [32]
        },
        "$listeners$": [[9, "resize", "handleResize"], [0, "burgerClick", "handleToggle"], [0, "leftSidebarClick", "handleToggle"]],
        "$lazyBundleIds$": "-",
        "$attrsToReflect$": []
    }; }
...
const apiEndpoint = "https://capacitor.cdn.prismic.io/api/v2" ;
...
const getBlogPost = (slug) => __awaiter(void 0, void 0, void 0, function* () {
    const prismicClient = Client();
    return prismicClient.getByUID('blog', slug, {});
});
const getBlogPosts = (_page = 0, pageSize = 10) => __awaiter(void 0, void 0, void 0, function* () {
    const prismicClient = Client();
    const res = yield prismicClient.query(Prismic.Predicates.at('document.type', 'blog'), { pageSize });
    return {
        docs: res.results,
        _prismic: Object.assign({}, res)
    };
});
...
function serializeHyperlink(linkResolver, element, children, key) {
    ...
    const props = Object.assign({ href: PrismicHelpers.Link.url(element.data, linkResolver) }, targetAttr, relAttr);
...
const PrismicRichText = (_a) => {
    var { richText, linkResolver, htmlSerializer } = _a, props = __rest$3(_a, ["richText", "linkResolver", "htmlSerializer"]);
    const serializedChildren = PrismicRichTextLib.serialize(richText, serialize.bind(null, linkResolver, {}), htmlSerializer);
    return h("div", Object.assign({}, applyProps(props)), serializedChildren);
};

const blogPageCss = "blog-page{display:block}blog-page h2{margin-top:0}blog-page a{text-decoration:none}blog-page .blog-post{margin:100px auto 0;max-width:670px;padding-bottom:100px}blog-page .blog-post p{color:#111}";
...
class BlogPage {
    ...
    componentWillLoad() {
        return __awaiter$1(this, void 0, void 0, function* () {
            ...
            if (slug) {
                this.slug = slug;
                this.post = yield getBlogPost(slug);
                console.log('Fetching blog post', slug, this.post);
            }
            else {
                this.posts = yield getBlogPosts();
            }
        });
    }
    render() {
...
const getBlogPostUrl = (doc) => `/url/${doc.data.slug}`;
const BlogPost = ({ post }) => (h("div", { class: "blog-post" }, h("h1", null, post.data.title), h(PrismicRichText, { richText: post.data.content }), h("disqus-comments", { url: getBlogPostUrl(post), id: post.uid })));
const AllPosts = ({ posts }) => posts.docs.map(p => h(BlogPost, { post: p, single: false }));

const capacitorSiteFooterCss = ".sc-capacitor-site-footer-h{display:block}";
...
class DocumentComponent {
    ...
    render() {
        ...
        return (h("div", { class: "container" }, h("app-burger", null), h("site-menu", { selectedParent: this.parent, siteStructureList: siteStructure }), h("app-marked", { fetchPath: this.item.filePath, renderer: (docsContent) => [
                h("stencil-route-title", { pageTitle: docsContent.title ? `${docsContent.title} - Capacitor` : 'Capacitor' }),
                h("div", { class: "doc-content" }, h("div", { class: "measure-lg" }, h("div", { innerHTML: docsContent.content }), h("h2", null, "Contributors"), h("contributor-list", { contributors: docsContent.contributors }), h("lower-content-nav", { next: this.nextItem, prev: this.prevItem }))),
...
const landingPageCss = ".sc-landing-page:root{--color-capacitor-blue:#119EFF;--color-woodsmoke:#16161D;--color-dolphin:#626177;--color-gunpowder:#505061;--color-manatee:#8888A2;--color-cadet-blue:#abb2bf;--color-whisper:#EBEBF7;--color-selago:#F4F4FD;--color-white-lilac:#f8f8fc;--color-white:#fff;--color-green-haze:#00AB47;--color-dodger-blue:#1d9aff;--color-dodger-blue-hover:rgba(#1d9aff, 0.2);--color-old-lace:#fdf5e4;--color-wheatfield:#F1E3C5;--color-pirate-gold:#9A6400;--button-shadow:0 8px 16px rgba(0,0,0,.1), 0 3px 6px rgba(0,0,0,.08);--button-shadow-hover:0 4px 6px rgba(0,0,0,.12), 0 1px 3px rgba(0,0,0,.08);--ease-out-expo:cubic-bezier(0.19, 1, 0.22, 1)}.sc-landing-page-h{display:block;background:#F5F9FC}.hero.sc-landing-page img.sc-landing-page{max-width:100%}";
...
class LandingPage {
    ...
    render() {
        return [
            h(MetaHead, null),
            h(ResponsiveContainer, null, h("section", { class: "hero" }, h(Grid, null, h(Col, { md: 6, sm: 6, xs: 6, cols: 12 }, h("hgroup", null, h("h1", null, "A cross-platform native runtime for web apps."), h("h3", null, "Capacitor turns any web app into a native app so you can run one app across iOS, Android, and the Web with the same code."), h(AnchorButton, { href: "/docs/getting-started/", id: "get-started" }, "Get Started"), h(AnchorButton, { href: "/docs/", id: "explore-docs" }, "Explore Docs"), h("h5", null, "Supports"), h("img", { alt: "Apple, Android, PWA", src: "/assets/img/supported-env.png" }))), h(Col, { md: 6, sm: 6, xs: 6, cols: 12 }, h("img", { src: "/assets/img/capacitor-hero.jpg" })))), h("section", { class: "points" }, h(Heading, { level: 2 }, "Why Capacitor?"), h(Grid, null, h(Col, { md: 4, sm: 4, xs: 4, cols: 6 }, h("div", null, h("h2", null, "Cross Platform"), h("p", null, "Build web apps that run equally well on iOS, Android, and as Progressive Web Apps"))), h(Col, { md: 4, sm: 4, xs: 4, cols: 6 }, h("div", { class: "points__item points__item--nativeaccess" }, h("h2", null, "Native Access"), h("p", null, "Access the full Native SDK on each platform, and easily deploy to App Stores (and the web!)"))), h(Col, { md: 4, sm: 4, xs: 4, cols: 6 }, h("div", { class: "points__item points__item--simple" }, h("h2", null, "Use with Ionic"), h("p", null, "Capacitor provides native functionality for web apps, and is optimized for Ionic Framework"))), h(Col, { md: 4, sm: 4, xs: 4, cols: 6 }, h("div", { class: "points__item points__item--webnative" }, h("h2", null, "Web Native"), h("p", null, "Build apps with standardized web technologies that will work for decades, and easily reach users on the app stores ", h("i", null, "and"), " the mobile web."))), h(Col, { md: 4, sm: 4, xs: 4, cols: 6 }, h("div", { class: "points__item points__item--extensible" }, h("h2", null, "Extensible"), h("p", null, "Easily add custom native functionality with a simple Plugin API, or use existing Cordova plugins with our compatibility layer."))), h(Col, { md: 4, sm: 4, xs: 4, cols: 6 }, h("div", { class: "points__item points__item--opensource" }, h("h2", null, "Open Source"), h("p", null, "Capacitor is completely open source (MIT) and maintained by ", h("a", { href: "http://ionicframework.com/" }, "Ionic"), " and its community.")))))),
...
const newsletterSignupCss = ":root{--color-capacitor-blue:#119EFF;--color-woodsmoke:#16161D;--color-dolphin:#626177;--color-gunpowder:#505061;--color-manatee:#8888A2;--color-cadet-blue:#abb2bf;--color-whisper:#EBEBF7;--color-selago:#F4F4FD;--color-white-lilac:#f8f8fc;--color-white:#fff;--color-green-haze:#00AB47;--color-dodger-blue:#1d9aff;--color-dodger-blue-hover:rgba(#1d9aff, 0.2);--color-old-lace:#fdf5e4;--color-wheatfield:#F1E3C5;--color-pirate-gold:#9A6400;--button-shadow:0 8px 16px rgba(0,0,0,.1), 0 3px 6px rgba(0,0,0,.08);--button-shadow-hover:0 4px 6px rgba(0,0,0,.12), 0 1px 3px rgba(0,0,0,.08);--ease-out-expo:cubic-bezier(0.19, 1, 0.22, 1)}.newsletter{background-color:#eeeff1;padding:80px 0}.newsletter .container{display:-ms-flexbox;display:flex}.newsletter form{display:-ms-flexbox;display:flex;-ms-flex-pack:end;justify-content:flex-end}.newsletter hgroup{-ms-flex:0 0 60%;flex:0 0 60%;padding-right:40px}.newsletter hgroup h2{margin:0 0 6px}.newsletter hgroup p{margin:0;line-height:23px}.newsletter form{-ms-flex:0 0 40%;flex:0 0 40%}.newsletter form input{padding:5px 10px 5px 16px;width:200px;min-height:calc(100% - 1px);background-color:#fff;border:none;border-radius:6px;border-top-right-radius:0;border-bottom-right-radius:0;font-size:16px;font-weight:400;color:#070D12;letter-spacing:-0.22px}.newsletter form button{background-color:#5EB6FC;color:white;border-radius:6px;border-top-left-radius:0;border-bottom-left-radius:0}.newsletter form button:hover{background:#33a4ff}.newsletter form button:focus{background:#1e9aff}@media screen and (max-width: 768px){.newsletter .container{-ms-flex-direction:column;flex-direction:column;text-align:center;-ms-flex-align:center;align-items:center;-ms-flex-pack:justify;justify-content:space-between}.newsletter hgroup,.newsletter form{-ms-flex:0 0 100%;flex:0 0 100%}.newsletter hgroup{padding:0}.newsletter form{margin-top:24px;width:100%;-ms-flex-pack:center;justify-content:center}}.landing-page .newsletter{background-color:#102331}";
...
class NewsletterSignup {
    ...
    render() {
        return (h("section", { class: "newsletter" }, h("div", { class: "container" }, h("hgroup", null, h("h2", null, "Subscribe to our newsletter"), h("p", null, "The latest Capacitor news and resources sent straight to your inbox.")), h("form", { action: "https://codiqa.createsend.com/t/t/s/flhuhj/", method: "post" }, h("div", { class: "input-with-button" }, h("input", { "aria-label": "Email address", type: "email", placeholder: "Email address", id: "fieldEmail", name: "cm-flhuhj-flhuhj", required: true }), h("button", { type: "submit" }, "Subscribe"))))));
    }
...
class PlatformBar {
    ...
    render() {
        return (h(Host, null, h(ResponsiveContainer, { class: "platform-bar__container" }, h("div", { class: "platform-bar__logo" }, h("img", { src: "/assets/ionic-logo.png", alt: "Ionic Logo" })), h("div", { class: "platform-bar__desc" }, "See how ", h("strong", null, this.productName), " fits into the ", h("a", { href: "https://ionic.io/" }, "Ionic Ecosystem")))));
    }
...
class SiteHeader {
    ...
    componentWillLoad() {
        return __awaiter$2(this, void 0, void 0, function* () {
            const ret = yield fetch("https://api.github.com/repos/ionic-team/capacitor");
    ...
    render() {
        return (h(ResponsiveContainer, { class: "site-header" }, h("a", { href: "/", class: "site-header__logo-link" }, h("img", { src: "/assets/img/logo-light.png", alt: "Capacitor Logo" })), h("div", { class: "site-header__menu" }, h("a", { href: "#features" }, "Features"), h("a", { href: "/docs" }, "Docs"), h("a", { href: "/community" }, "Community"), h("a", { href: "/blog" }, "Community"), h("a", { href: "/enterprise" }, "Enterprise")), h("div", { class: "site-header__buttons" }, h(AnchorButton, { href: "https://github.com/ionic-team/capacitor", class: "site-header__buttons__github" }, h("ion-icon", { name: "logo-github" }), this.starCount ? this.starCount : 'GitHub'), h(Button, { class: "site-header__buttons__install" }, "Install"))));
    }
    get el() { return getElement(this); }
...
const siteMenuCss = ":root{--color-capacitor-blue:#119EFF;--color-woodsmoke:#16161D;--color-dolphin:#626177;--color-gunpowder:#505061;--color-manatee:#8888A2;--color-cadet-blue:#abb2bf;--color-whisper:#EBEBF7;--color-selago:#F4F4FD;--color-white-lilac:#f8f8fc;--color-white:#fff;--color-green-haze:#00AB47;--color-dodger-blue:#1d9aff;--color-dodger-blue-hover:rgba(#1d9aff, 0.2);--color-old-lace:#fdf5e4;--color-wheatfield:#F1E3C5;--color-pirate-gold:#9A6400;--button-shadow:0 8px 16px rgba(0,0,0,.1), 0 3px 6px rgba(0,0,0,.08);--button-shadow-hover:0 4px 6px rgba(0,0,0,.12), 0 1px 3px rgba(0,0,0,.08);--ease-out-expo:cubic-bezier(0.19, 1, 0.22, 1)}site-menu{display:block;-ms-flex:0 0 auto;flex:0 0 auto;margin-top:100px}site-menu .section-label{color:var(--color-woodsmoke);margin-bottom:0;font-size:14px;font-weight:600}site-menu .menu-list li,site-menu .menu-list ul li{list-style-type:none;margin:0;padding:0}site-menu .menu-list{margin-top:0;padding:0}site-menu .menu-list .section-label:first-of-type{margin-top:0;margin-bottom:0}site-menu .menu-list ul{padding:0;margin-top:0;margin-bottom:0}site-menu .menu-list li{font-size:14px}site-menu .menu-list>li+li{margin-top:8px}site-menu .menu-list a{font-weight:400;color:#6c6c8b;text-decoration:none;border:0}site-menu .menu-list a:hover{border:0}site-menu .menu-list .link-active{font-weight:500;color:var(--color-dodger-blue)}site-menu .menu-list a:hover:not(.link-active){color:var(--color-woodsmoke)}site-menu .menu-list ul li{padding-left:8px;-webkit-transition:80ms height;transition:80ms height;height:26px;display:block;overflow:hidden}site-menu .menu-list ul li a{display:block;-webkit-transition:0.2s color ease, 0.2s -webkit-transform ease;transition:0.2s color ease, 0.2s -webkit-transform ease;transition:0.2s transform ease, 0.2s color ease;transition:0.2s transform ease, 0.2s color ease, 0.2s -webkit-transform ease}site-menu .menu-list ul li:hover a,site-menu .menu-list ul li a.link-active{-webkit-transform:translateX(2px);transform:translateX(2px)}site-menu .menu-list ul.collapsed li{height:0}@media screen and (max-width: 768px){site-menu{position:fixed;top:0;left:0;background:var(--color-woodsmoke);z-index:999;padding:20px;width:calc(100vw - 56px);-webkit-transform:translateX(calc(-100vw + 56px));transform:translateX(calc(-100vw + 56px));height:100%;overflow-y:scroll}site-menu .menu-list .section-label{color:white}site-menu .menu-list a{color:rgba(255, 255, 255, 0.6)}site-menu .menu-list a:hover:not(.link-active){color:white}}";
...
class SiteMenu {
    ...
    render() {
        return (h("div", { class: "sticky" }, h("div", null, h("ul", { class: 'menu-list' }, this.siteStructureList.map((item, i) => (h("li", null, h("a", { href: "#", onClick: this.toggleParent(i) }, h("span", { class: "section-label" }, item.text)), h("ul", { class: { 'collapsed': this.closeList.indexOf(i) !== -1 } }, item.children.map((childItem) => (h("li", null, (childItem.url) ?
            h("a", { href: childItem.url, onClick: () => state.isLeftSidebarIn = !state.isLeftSidebarIn }, childItem.text) :
            h("a", { rel: "noopener", class: "link--external", target: "_blank", href: childItem.filePath }, childItem.text))))))))))));
    }
    static get style() { return siteMenuCss; }
    static get cmpMeta() { return {
        "$flags$": 0,
        "$tagName$": "site-menu",
        "$members$": {
